...
> **Note:** All IDs in this codebase are placeholders. Replace customer IDs, playlist IDs, and spreadsheet URLs with your actual values before use. All tokens used here are accessbile to anyone with the same rights in G Ads as you, so use access tokens at your own risk.  
...
1. Go to G Ads -> Tools -> Bulk actions -> Scripts  
2. Copy each `.gs` file content into separate files in the editor (keep the same names)
3. Update `Config.gs` with your playlist IDs, spreadsheet URL, and Google Ads IDs
4. Allow YT API access within G Ads Scripts by selecting YouTube from Script -> Advanced APIs
5. Click "Run" and you'll be asked for a browser authorization for G Ads and YouTUbe
6. Re-run if necessary to launch it.
9. Set Frequency to launch scripts automatically 
...
## License
Free to use if you buy me a beer next time you see me. 
